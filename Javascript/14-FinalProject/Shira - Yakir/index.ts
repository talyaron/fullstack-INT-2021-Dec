...
function tasksPage() {
  let today = new Date();
  let todayDisplay = `${today.getDate()}/${
    today.getMonth() + 1
  }/${today.getFullYear()}`;
...
function submitEditTaskForm(event) {
  ...
  let editTaskObj = new task(
    event.target.editTaskTitle.value,
    event.target.editContent.value,
    new Date(event.target.editDueDate.value),
    false,
    choice
  );
...
function selctCtgEdit(event) {
  choice = event.target.value;
  let selectCatgEditForm: any = document.querySelector(".editCategory");
